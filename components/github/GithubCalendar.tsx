...
export default function GithubCalendar({ data, colorScheme = "green" }: Props) {
  ...
  return (
    <div className="flex flex-col text-gray-300 text-xs">
      {/* Label bulan */}
      <div className="flex ml-10 space-x-[2px] mb-1">
        {monthLabels.map((m, idx) => (
          <span key={idx} className="w-[10px] text-center">{m}</span>
        ))}
      </div>

      <div className="flex">
        {/* Label hari */}
        <div className="flex flex-col justify-between h-[70px] mr-1">
          {dayLabels.map((d, idx) => (
            <span key={idx} className="text-[10px]">{d}</span>
          ))}
        </div>
          ...
          {weeks.map((week, wIdx) => (
            <div key={wIdx} className="flex flex-col space-y-[2px]">
              {week.map((day) => (
                <div
                  key={day.date}
                  className={`w-[10px] h-[10px] ${getColor(day.count)} rounded-sm`}
      ...
      <div className="flex items-center space-x-1 mt-2 text-[10px]">
        ...
        <div className="flex space-x-[2px]">
          <div className={`w-[10px] h-[10px] bg-gray-800 rounded-sm`} />
          <div className={`w-[10px] h-[10px] ${getColor(2)} rounded-sm`} />
          <div className={`w-[10px] h-[10px] ${getColor(5)} rounded-sm`} />
          <div className={`w-[10px] h-[10px] ${getColor(10)} rounded-sm`} />
        </div>
        <span>More</span>
      </div>
    </div>

...
export const ExperienceCard = ({ experience, showDetail = false }: { experience: Experience, showDetail?: boolean}) => {
  ...
  const endDate = experience.end_date ? formatDate({ 
    ...
    <div className="border border-primary p-5 sm:p-6 rounded-2xl flex flex-col bg-primary">
      <div className="flex justify-between gap-3 items-start">
        <div className="flex flex-col md:flex-row gap-3 md:gap-6">
          <div className="relative h-12 w-12 md:h-20 md:w-20 shrink-0 overflow-hidden rounded-full border border-primary">
            <Image
              src={logoSrc}
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <p className="text-secondary text-sm flex flex-wrap gap-y-0.5 gap-x-3 items-center md:mt-0">
              ...
              <span className="flex items-center gap-0.5"><LuMapPin className="shrink-0"/>{experience.location}</span> 
              <span className="flex items-center gap-0.5"><LuBriefcaseBusiness className="shrink-0"/>{experience.employment_type}</span> 
            </p>
          </div>
        </div>
